...
questionFiles.forEach(filePath => {
  ...
  if (filename.includes('-Explanation')) {
    ...
    const answerLst = answerMap.get(title) ?? answerMap.set(title, []).get(title);
    answerLst!!.push(`**${answerNum}.** ${answer}\n\n`);
  } else {
    const questionNum = filename.split('.')[0];
    const question = fs.readFileSync(absolutePath, 'utf-8');
    const questionLst = questionMap.get(title) ?? questionMap.set(title, []).get(title);
...
createMarkdownFile(markdownOutputDir, `${repoBasename}-questions.md`, questionMap);
...
function createMarkdownFile(dir: string, filename: string, dataMap: Map<string, string[]>): void {
  fs.rmSync(filename, { force: true });

  const output = fs.createWriteStream(path.join(dir, filename));
  output.write(filename.includes('questions') ? `#QUESTIONS - ${filename}\n` : `#ANSWERS - ${filename}\n`);
